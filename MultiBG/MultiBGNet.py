...
class MBGInjectNet(nn.Module):
    def __init__(self, net1, net2):
        super(MBGInjectNet, self).__init__()
        self.net1 = net1
        self.net2 = net2
...
class MutiBGNet:
    ...
    def train(self):
        ...
        # 保存神经网络
        torch.save(model, 'MUTNet2.pkl')  # 保存整个神经网络的结构和模型参数
        # torch.save(model.state_dict(), 'MUTNet_params.pkl')  # 只保存神经网络的模型参数
    ...
    def val_document(self):
        ...
        train_dataset = torchvision.datasets.ImageFolder(
            root='', transform=data_transform)
        train_dataset_loader = DataLoader(
            train_dataset, batch_size=25, shuffle=True, num_workers=4)
        correct = 0
    ...
    def inject(self):
        model = torch.load('MUTNet2.pkl')
        targetmodel = torchvision.models.inception_v3(pretrained=True)
        injectmodel = MBGInjectNet(model, targetmodel)
        return injectmodel
